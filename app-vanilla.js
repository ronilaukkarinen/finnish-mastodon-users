...
addEventListener('DOMContentLoaded', () => {
  ...
  fetch("following_accounts.csv", { cache: "force-cache" })
  ...
  .then(result => {
    ...
    // Cache lines to local storage for 15 minutes
    localStorage.setItem("following_accounts", lines);
    localStorage.setItem("following_accounts_timestamp", Date.now());

    // Expire cache after 15 minutes
    if (localStorage.getItem("following_accounts_timestamp") + 900000 < Date.now()) {
      localStorage.removeItem("following_accounts");
      localStorage.removeItem("following_accounts_timestamp");
    }

    // Serve from cache if available
    if (localStorage.getItem("following_accounts")) {
      lines = localStorage.getItem("following_accounts");
